...
export async function POST(
  ...
  try {
    ...
    console.error(`Error in POST /api/archive/:id:`, error);
    return NextResponse.json(
      {
        success: false,
...
// Handle DELETE requests to delete a specific archived document
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({
        error: { message: 'Failed to delete archived document' },
      }));

      return NextResponse.json(
        {
          success: false,
          error: errorData.error || { message: 'Failed to delete archived document' },
        },
        { status: response.status }
      );
    }
    ...
    console.error('Error in DELETE /api/archive/:id:', error);
    return NextResponse.json(
      {
        success: false,
        error: { message: error.message || 'Internal server error' },
