...
import { Shield, Download, Search, Calendar, User, FileText, Loader2, RefreshCw, FileTextIcon, FileSpreadsheet, FileImage } from "lucide-react";
...
export default function SigningHistoryPage() {
  ...
  const { data, loading: dataLoading, error, refetch } = useSigningHistory(undefined, filter === "all" ? undefined : filter);
  ...
  const filteredHistory = useMemo(() => {
    ...
    return data.signingHistory.filter(entry =>
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        filteredHistory.forEach((entry) => {
          csvContent += `"${entry.document}","${entry.documentCode}","${entry.signer}","${entry.department}","${new Date(entry.timestamp).toLocaleString()}","${entry.txHash}","${entry.status}"\n`;
        });
        ...
        link.setAttribute('download', `signing-history-report-${new Date().toISOString().split('T')[0]}.csv`);
        ...
        const historyData = [
          ['Document', 'Document Code', 'Signer', 'Department', 'Timestamp', 'Transaction Hash', 'Status']
        ];
        filteredHistory.forEach((entry) => {
          historyData.push([
            entry.document,
        ...
        // Write the file
        writeFile(wb, `signing-history-report-${new Date().toISOString().split('T')[0]}.xlsx`);
      } else if (format === 'pdf') {
        ...
        doc.text('Signing History Report', pageWidth / 2, margin + 10, { align: 'center' });

        // Add filter info
        doc.setFontSize(12);
        doc.setTextColor(107, 114, 128);
        doc.text(`Filter: ${filter === 'all' ? 'All Signatures' : filter.charAt(0).toUpperCase() + filter.slice(1)}`, pageWidth / 2, margin + 18, { align: 'center' });
        ...
        if (filteredHistory.length > 0) {
          const historyData = filteredHistory.map((entry) => [
            entry.document.substring(0, 30) + (entry.document.length > 30 ? '...' : ''),
            entry.documentCode,
            entry.signer,
            entry.department,
          ...
          (doc as any).autoTable({
            startY: currentY,
            head: [['Document', 'Code', 'Signer', 'Department', 'Date', 'TX Hash', 'Status']],
            body: historyData,
            theme: 'grid',
            headStyles: {
              fillColor: [37, 99, 235],
        ...
        // Save the PDF
        doc.save(`signing-history-report-${new Date().toISOString().split('T')[0]}.pdf`);
      }
    } catch (err: any) {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8">
        <div className="text-red-500 text-lg mb-4">Error loading signing history</div>
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="relative group inline-block">
          <Button disabled={loading || dataLoading} className="w-full sm:w-auto">
            {loading || dataLoading ? (
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data?.statistics.totalSignatures || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data?.statistics.thisWeek || 0}</div>
            <p className="text-xs text-muted-foreground">Signatures in the last 7 days</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data?.statistics.successRate || '100%'}</div>
              ...
              {filteredHistory.map((entry) => (
                <div key={entry.id} className="flex items-center justify-between border-b pb-4 last:border-0">
                  <div className="space-y-1">
                    ...
                    <code className="text-xs text-muted-foreground break-all">{entry.txHash}</code>
                  </div>
