...
import { Shield, CheckCircle, XCircle, AlertTriangle, Download, Calendar, FileText, Loader2, X } from "lucide-react";
...
export default function ComplianceReportsPage() {
  ...
  const { data: scheduledReports, refetch: refetchScheduled } = useScheduledReports();
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <h2 className="text-xl font-semibold mb-2">Error Loading Compliance Report</h2>
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <p className="text-muted-foreground">There is no compliance data to display at this time.</p>
      </div>
    );
  }
  ...
  const complianceMetricItems = [
    {
      ...
      percentage: Math.round(Number(complianceMetrics.complianceRate.replace('%', '')))
    },
    {
      label: "Pending Signatures",
      value: complianceMetrics.pendingSignatures.toString(),
      total: complianceMetrics.totalDocuments.toString(),
      percentage: Math.round((complianceMetrics.pendingSignatures / complianceMetrics.totalDocuments) * 100)
    },
    {
    ...
    },
    {
      ...
      status: complianceMetrics.failedVerifications > 0 ? "warning" : "excellent"
    }
  ];
  ...
  const handleExport = async (format: 'pdf' | 'csv' | 'excel') => {
    try {
      ...
      if (format === 'csv') {
        ...
        csvContent += `Documents Signed,${currentData.complianceMetrics.documentsSigned?.toString() || '0'}\n`;
        csvContent += `Total Documents,${currentData.complianceMetrics.totalDocuments?.toString() || '0'}\n`;
        csvContent += `Compliance Rate,${currentData.complianceMetrics.complianceRate || '0%'}\n`;
        csvContent += `Pending Signatures,${currentData.complianceMetrics.pendingSignatures?.toString() || '0'}\n`;
        csvContent += `Failed Verifications,${currentData.complianceMetrics.failedVerifications?.toString() || '0'}\n\n`;
        ...
        const statsData = [
          ['Metric', 'Value'],
          ['Documents Signed', currentData.complianceMetrics.documentsSigned?.toString() || '0'],
          ['Total Documents', currentData.complianceMetrics.totalDocuments?.toString() || '0'],
          ['Compliance Rate', currentData.complianceMetrics.complianceRate || '0%'],
          ['Pending Signatures', currentData.complianceMetrics.pendingSignatures?.toString() || '0'],
          ['Failed Verifications', currentData.complianceMetrics.failedVerifications?.toString() || '0'],
        ];
        ...
        if (currentData.pendingSignatures) {
          currentData.pendingSignatures.forEach(sig => {
            pendingSignaturesData.push([sig.document, sig.documentCode, sig.daysOverdue.toString(), sig.priority]);
          });
        }

        const recentSignaturesData = [
          ['Document', 'Signer', 'Date', 'Status']
        ];
        if (currentData.recentSignatures) {
          currentData.recentSignatures.forEach(sig => {
            recentSignaturesData.push([sig.document, sig.signer, sig.date, sig.status]);
          });
        }
        ...
        // Write the file
        writeFile(wb, `compliance-report-${new Date().toISOString().split('T')[0]}.xlsx`);
      } else if (format === 'pdf') {
        ...
        doc.text('Compliance Report', pageWidth / 2, margin + 10, { align: 'center' });

        // Add subtitle
        doc.setFontSize(12);
        doc.setTextColor(107, 114, 128); // gray-500
        doc.text('Digital signature compliance and audit metrics', pageWidth / 2, margin + 18, { align: 'center' });
        ...
        const statsData = [
          ['Documents Signed', currentData.complianceMetrics.documentsSigned?.toLocaleString() || '0'],
          ['Total Documents', currentData.complianceMetrics.totalDocuments?.toLocaleString() || '0'],
          ['Compliance Rate', currentData.complianceMetrics.complianceRate || '0%'],
          ['Pending Signatures', currentData.complianceMetrics.pendingSignatures?.toLocaleString() || '0'],
          ['Failed Verifications', currentData.complianceMetrics.failedVerifications?.toLocaleString() || '0'],
        ];
        ...
        if (currentData.pendingSignatures && currentData.pendingSignatures.length > 0) {
        ...
        if (currentData.recentSignatures && currentData.recentSignatures.length > 0) {
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(result.message || `Failed to schedule report: ${response.status}`);
      }
      ...
      setScheduleError(err.message || 'Failed to schedule report. Please try again.');
    } finally {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Digital signature compliance and audit metrics</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowScheduledReportsModal(true)}>
      ...
      {pendingSignatures.length > 0 && (
        <Alert className="border-yellow-200 bg-yellow-50">
          ...
          <AlertDescription>
            <strong>{pendingSignatures.length} documents</strong> are pending digital signatures.
          ...
          <CardContent>
            <div className="space-y-3">
              {pendingSignatures.length > 0 ? (
                pendingSignatures.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Code: {item.documentCode}</p>
                    </div>
                ...
                <p className="text-muted-foreground text-center py-4">No pending signatures</p>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {recentSignatures.length > 0 ? (
                ...
                <p className="text-muted-foreground text-center py-4">No recent signatures</p>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {data.timeline && data.timeline.length > 0 ? (
              ...
              <p className="text-muted-foreground text-center py-4">No compliance events to display</p>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      {scheduledReports && scheduledReports.filter(r => r.type === 'compliance' && r.isActive).length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
                  ...
                  <div key={schedule.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="space-y-1">
                      ...
                      <div className="text-sm text-muted-foreground">
                        {schedule.schedule && (
                          <>
                            ...
                            {schedule.schedule.frequency === 'weekly' && `Weekly (${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][schedule.schedule.day || 0]})`}
                            {schedule.schedule.frequency === 'monthly' && `Monthly (Day ${schedule.schedule.day || 1})`}
                      ...
                      {schedule.reportGeneratedAt && (
                        <div className="text-xs text-muted-foreground">
                          Generated: {new Date(schedule.reportGeneratedAt).toLocaleString()}
                        </div>
                      )}
                    </div>
                    {schedule.reportFileName && (
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            const response = await fetch(`/api/reports/scheduled/${schedule.id}/download`);
                            if (!response.ok) throw new Error('Failed to download');
                            const blob = await response.blob();
                            const url = window.URL.createObjectURL(blob);
                            const a = document.createElement('a');
                            a.href = url;
                            a.download = schedule.reportFileName || `compliance-report-${schedule.id}.pdf`;
            ...
            <div className="grid gap-4">
              ...
              {(scheduleFrequency === 'weekly' || scheduleFrequency === 'monthly') && (
                <div>
                  ...
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    {scheduleFrequency === 'weekly' ? '0 = Sunday, 1 = Monday, etc.' : 'Enter day of the month (1-31)'}
                  </p>
                </div>
              )}
